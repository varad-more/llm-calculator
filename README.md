...
result is labelled `predicted`. The harness is built and tested: it parses vLLM and SGLang startup
output, diffs each component against our formulas, and fails CI outside ±1% (weights) / ±5% (KV) / ±15% (overhead). It just has nothing to chew on.
...
The weights *are* measured today. The analytic parameter count is checked against each
...
- **Size a config.** Pick a model, GPU, engine and workload, and watch the per-GPU allocation
  ...
  prefill/decode with the KV bytes that cross the wire. Every assumption is editable in place. The
  config lives in the query string, so a sizing is a link you can paste.
...
- **How it works** (`/explained/`). The method, written out: what quantization actually stores and
  ...
  figure is computed at build time by the same functions the sizer calls, so the documentation
...
carries the layout it needs, whether it fits, its TTFT, and what a million tokens costs. Input and
output are priced separately, because the two differ by more than an order of magnitude. Type a
latency target and the table narrows to the machines that hold it. Type your own spot rate over the
...
**4.3x** the throughput of eight TP=1 replicas. A decode step streams the sharded weights once and
serves the whole batch from that read, where replicas each re-read the entire model. The crossover
where replicas win is real, past ~1k concurrent sequences on an 8B model, and the search finds it
instead of encoding a rule of thumb. See [docs/MATH.md](docs/MATH.md#machine-search).

Local hardware is in the same catalogue as the datacentre parts: Apple M2/M3/M4/M5 Ultra, Max and
Pro, and GeForce RTX 3090/4090/5090, alongside H100s and MI300X. Unified memory is modelled the way
Metal actually hands it out: a process gets `recommendedMaxWorkingSetSize`, about 75% of RAM. That
is why a 128GB M4 Max shows 96 GiB usable, and why a 512GB M5 Ultra cannot quite hold DeepSeek-V3
at Q4_K_M without raising `iogpu.wired_limit_mb`.
