...
| `rtx4090-24` | 24 GiB | GDDR6X | 1.01 TB/s | 165.2 | 330.3 | pcie4 @ 32 GB/s | [datasheet](https://images.nvidia.com/aem-dam/Solutions/geforce/blackwell/nvidia-rtx-blackwell-gpu-architecture.pdf) |
...
| `rtx5090-32` | 32 GiB | GDDR7 | 1.79 TB/s | 209.5 | 419 | pcie5 @ 64 GB/s | [datasheet](https://images.nvidia.com/aem-dam/Solutions/geforce/blackwell/nvidia-rtx-blackwell-gpu-architecture.pdf) |
| `rtx3090-24` | 24 GiB | GDDR6X | 0.94 TB/s | 71.2 | — | pcie4 @ 32 GB/s | [datasheet](https://images.nvidia.com/aem-dam/Solutions/geforce/blackwell/nvidia-rtx-blackwell-gpu-architecture.pdf) |

**rtx4090-24:** fp16/bf16/fp8 are the FP32-accumulate dense figures, which is the serving path. FP16-accumulate peaks higher (330.3 bf16, 660.6 fp8) but is not what inference kernels use. int8 is the whitepaper's dense INT8 TOPS.
...
**rtx5090-32:** Tensor TFLOPS are the FP32-accumulate figures, which is what serving kernels use, and the dense half of the whitepaper's dense/sparse pair. GeForce has no NVLink, so multi-GPU is over PCIe and the all-reduce term dominates prefill at higher tp.

**rtx3090-24:** Tensor TFLOPS are the FP32-accumulate figures, which is what serving kernels use, and the dense half of the whitepaper's dense/sparse pair. GeForce has no NVLink, so multi-GPU is over PCIe and the all-reduce term dominates prefill at higher tp. Ampere has no FP8 tensor path, so fp8 is absent rather than approximated.
